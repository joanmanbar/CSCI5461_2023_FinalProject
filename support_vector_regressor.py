...
#
CircadianRegions = CircadianRegions.sample(frac=0.7, replace=False, random_state=5461) # Sample a fraction of the df
#
#********************************************************
...
data_df = data_df.dropna(subset=['JTK_adjphase']) #Dropping NAs in dataset
...
predictions_df.to_csv('predicted_values_svr_.csv', index=False)
print('kernel is rbf')

# Using permutation importance to get the most important features
...
r = permutation_importance(estimator=svr, X=X_train_val, y=y_train_val, n_repeats=3, random_state=7)
