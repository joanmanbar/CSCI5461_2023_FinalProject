...
# Random Forest Model
rf = RandomForestRegressor(n_estimators=100, max_depth=400, random_state=5461, verbose=3)
n_folds = 3
print("before Cross Validation")
cv = run_cv(n_folds, rf, X_train, y_train)
print("Cross Validation done")
print("\nFor 100 trees and max depth of 400:")
print(cv)
cross_val_df = pd.DataFrame(cv)
cross_val_df.to_csv('cross_val_rf1.csv', index=False)
...
predictions_df.to_csv('predicted_values_rf_cv1.csv', index=False)
...
# save the dataframe to a csv file
feat_imp_df.to_csv('feature_importance_rf_cv1.csv', index=False)
