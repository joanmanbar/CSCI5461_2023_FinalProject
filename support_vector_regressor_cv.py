...
#
CircadianRegions = CircadianRegions.sample(frac=0.75, replace=False, random_state=5461) # Sample a fraction of the df
#
#********************************************************
...
cv = run_cv(n_folds, svr, X_train, y_train)
print("Cross Validation done")
print("\nFor C=1:")
print(cv)
cross_val_df = pd.DataFrame(cv)
cross_val_df.to_csv('cross_val_svr.csv', index=False)
...
predictions_df.to_csv('predicted_values_svr_cv2.csv', index=False)
...
r = permutation_importance(estimator=svr, X=X_train_train, y=y_train_train, n_repeats=3, random_state=5461)
...
# save the dataframe to a csv file
feat_imp_df.to_csv('feature_importance_svr_cv75.csv', index=False)
...
# fig.savefig('feature_importance_svr.png', dpi=150)
